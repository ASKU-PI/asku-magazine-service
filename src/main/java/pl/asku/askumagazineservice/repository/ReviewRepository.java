...
public interface ReviewRepository extends CrudRepository<Review, Long> {
    ...
    @Query("SELECT ROUND(AVG(rw.rating), 1) FROM Review rw JOIN Reservation rn ON rn.id = rw.reservation.id JOIN Magazine m ON " +
            "m.id = rn.magazine.id WHERE m.id = :magazineId")
